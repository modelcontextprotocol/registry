...
//   - Maven: RegistryType, Identifier ("groupId:artifactId"), Version, RegistryBaseURL (optional)
type Package struct {
	// RegistryType indicates how to download packages (e.g., "npm", "pypi", "oci", "nuget", "mcpb", "maven")
	RegistryType string `json:"registryType" minLength:"1" doc:"Registry type indicating how to download packages (e.g., 'npm', 'pypi', 'oci', 'nuget', 'mcpb', 'maven')" example:"npm"`
	// RegistryBaseURL is the base URL of the package registry (used by npm, pypi, nuget, maven; not used by oci, mcpb)
	...
	//   - For Maven: "groupId:artifactId" coordinate (e.g., "org.example:my-mcp-server")
	...
	// Version is the package version (required for npm, pypi, nuget, maven; optional for mcpb; not used by oci where version is in the identifier)
