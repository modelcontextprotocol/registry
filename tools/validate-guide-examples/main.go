// validate-guide-examples validates JSON examples in docs/guides/publishing/publish-server.md
// that contain a $schema field against both schema.json and registry-schema.json.
package main
...
func runValidation() error {
	guidePath := filepath.Join("docs", "guides", "publishing", "publish-server.md")
	schemaPath := filepath.Join("docs", "reference", "server-json", "server.schema.json")

	examples, err := extractExamples(guidePath)
	if err != nil {
		return fmt.Errorf("failed to extract examples: %w", err)
	}

	log.Printf("Found %d JSON examples with $schema in publish-server.md\n", len(examples))

	if len(examples) == 0 {
		log.Println("No examples with $schema found - this is expected if none exist")
		return nil
	}
...
func extractExamples(path string) ([]example, error) {
	...
	// Regex to match JSON code blocks in markdown that contain $schema
	// First find all JSON code blocks, then filter for those with $schema
	...
	for _, match := range matches {
		...
		// Only include examples that contain $schema
		if strings.Contains(jsonContent, "$schema") {
